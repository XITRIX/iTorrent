...
//    }
    ...
    /**
     Get peers of torrent by hash in queue.
     - Parameter hash: torrent's hash.
     - Returns: An array of peers.
     */
    public static func getPeers(with hash: String) -> [PeerModel] {
        let rawRes = get_peers_by_hash(hash)
        let res = Array(UnsafeBufferPointer(start: rawRes.peers, count: Int(rawRes.count))).map { PeerModel($0) }.sorted(by: { $0.address < $1.address })
        free_peer_result(rawRes)
        return res
    }
}
