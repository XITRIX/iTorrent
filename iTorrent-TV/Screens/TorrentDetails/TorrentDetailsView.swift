...
struct TorrentDetailsView: View {
    let infoHashes: TorrentSession.Hashes

    @EnvironmentObject private var torrentService: TorrentService

    private var torrentHandle: TorrentSession.Handle? {
        torrentService.modernHandles[infoHashes]
    }

    private var snapshot: TorrentSession.Handle.Snapshot? {
        torrentHandle?.currentSnapshot
    }

    var body: some View {
        Group {
            if let torrentHandle, let snapshot {
                TabView {
                    Tab {
                        List {
                            Section {
                ...
                ContentUnavailableView("Torrent not found", systemImage: "externaldrive.badge.xmark")
            }
        }
    }
}
